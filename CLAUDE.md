...
- `scan.go` — Main scanning command with `--dump-keys`, `--dump-certs`, `--max-file-size`, `--export` flags. Contains `formatDN()` helper for OpenSSL-style distinguished name formatting.
...
- **Never write files without explicit consent.** Commands that produce PEM output print to stdout by default. Files are only written when the user provides `-o`. Export (`--export`) requires an explicit `-o` path. No silent writes to the current directory.
...
- **Expired certificates are skipped** during ingestion by default (not stored in DB). Set `Config.IncludeExpired = true` to retain them (used by `--dump-certs`).
